...
export function MatrixOperations({ variables, onEditVariable, useFractions }: MatrixOperationsProps) {
  ...
  const calculateOperation = async () => {
    ...
    // Validaciones básicas según tipo de operación
    if (
      (operation === 'transpose' || operation === 'inverse' || operation === 'scalar') &&
      (!first || !first.matrix)
    ) {
      setError('Debe seleccionar una matriz válida.');
      return;
    }
    ...
    if (
      ...
      setError('Debe seleccionar ambas matrices.');
      return;
    }
    ...
    // Validación extra para inversa (backend también valida, pero así damos feedback inmediato)
    if (operation === 'inverse' && first && first.rows !== first.cols) {
      setError('La matriz debe ser cuadrada para calcular su inversa.');
      return;
    }
    ...
    try {
      const payload = {
        operation,
        firstMatrix: first?.matrix ?? null,
        secondMatrix: second?.matrix ?? null,
        scalar,
      };
