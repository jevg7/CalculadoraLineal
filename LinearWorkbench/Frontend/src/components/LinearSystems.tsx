...
export function LinearSystems({ variables, onEditVariable, useFractions }: LinearSystemsProps) {
  ...
  const solveSystem = (augmentedMatrix: Matrix): Solution => {
    const steps: string[] = [];
    ...
    steps.push('════════════════════════════════════════════');
    steps.push('');
    steps.push('Objetivo: Reducir la matriz aumentada [A|b] a forma escalonada');
    steps.push('reducida [I|x] para encontrar la solución del sistema.');
    steps.push('');
    steps.push('Matriz aumentada inicial [A|b]:');
    ...
    for (let col = 0; col < m - 1 && currentRow < n; col++) {
      steps.push(`──────────────────────────────────────────────`);
      steps.push(`Trabajar con columna ${col + 1}`);
      steps.push(`──────────────────────────────────────────────`);
      steps.push('');
      ...
      if (Math.abs(matrix[pivotRow][col]) < 0.0001) {
        steps.push(`El pivote en columna ${col + 1} es ≈ 0, pasar a siguiente columna`);
        steps.push('');
        continue;
      }

      steps.push(`Buscar pivote máximo en columna ${col + 1}:`);
      steps.push(`  Pivote seleccionado: elemento (${pivotRow + 1}, ${col + 1}) = ${formatNumber(matrix[pivotRow][col], useFractions)}`);
      steps.push('');

      // Intercambiar filas
      if (pivotRow !== currentRow) {
        [matrix[currentRow], matrix[pivotRow]] = [matrix[pivotRow], matrix[currentRow]];
        steps.push(`Intercambiar filas para colocar el pivote:`);
        steps.push(`  F${currentRow + 1} ↔ F${pivotRow + 1}`);
        steps.push('');
        steps.push('Matriz después del intercambio:');
        steps.push(formatMatrix(matrix));
        steps.push('');
      }
      ...
      if (Math.abs(pivot - 1) > 0.0001) {
        steps.push(`Normalizar fila ${currentRow + 1} (hacer el pivote = 1):`);
        steps.push(`  F${currentRow + 1} ÷ ${formatNumber(pivot, useFractions)}`);
        ...
        steps.push('');
        steps.push('Matriz después de normalizar:');
        steps.push(formatMatrix(matrix));
        steps.push('');
      }

      // Eliminar columna
      steps.push(`Hacer ceros en el resto de la columna ${col + 1}:`);
      ...
      for (let row = 0; row < n; row++) {
        if (row !== currentRow && Math.abs(matrix[row][col]) > 0.0001) {
          ...
          steps.push(`  F${row + 1} ← F${row + 1} - (${formatNumber(factor, useFractions)}) × F${currentRow + 1}`);
      ...
      if (anyElimination) {
        steps.push('');
        steps.push('Matriz después de eliminación:');
        steps.push(formatMatrix(matrix));
        steps.push('');
      } else {
        steps.push('  (La columna ya tiene ceros en las otras posiciones)');
        steps.push('');
      }
    ...
    steps.push('════════════════════════════════════════════');
    steps.push('   FORMA ESCALONADA REDUCIDA ALCANZADA');
    steps.push('════════════════════════════════════════════');
    steps.push('');
    steps.push('Matriz final:');
    steps.push(formatMatrix(matrix));
    steps.push('');
    steps.push('──────────────────────────────────────────────');
    steps.push('ANÁLISIS DE LA SOLUCIÓN:');
    steps.push('──────────────────────────────────────────────');
    steps.push('');
    ...
    for (let i = 0; i < n; i++) {
      const allZeros = matrix[i].slice(0, m - 1).every(v => Math.abs(v) < 0.0001);
      const constantNonZero = Math.abs(matrix[i][m - 1]) > 0.0001;

      if (allZeros && constantNonZero) {
        hasContradiction = true;
        steps.push(`✗ CONTRADICCIÓN detectada en fila ${i + 1}:`);
        steps.push(`  0 = ${formatNumber(matrix[i][m - 1], useFractions)}`);
        steps.push('');
        steps.push('Conclusión: El sistema es INCONSISTENTE.');
        steps.push('No existe solución que satisfaga todas las ecuaciones.');
        steps.push('');
        steps.push('════════════════════════════════════════════');
        break;
      }
    ...
    const leadingOnes = new Set<number>();
    for (let i = 0; i < n; i++) {
      for (let j = 0; j < m - 1; j++) {
        if (Math.abs(matrix[i][j] - 1) < 0.0001) {
          const isLeading = matrix[i].slice(0, j).every(v => Math.abs(v) < 0.0001);
          if (isLeading) {
            leadingOnes.add(j);
            break;
          }
    ...
    freeVariables = (m - 1) - leadingOnes.size;

    if (freeVariables > 0) {
      steps.push(`Número de variables: ${m - 1}`);
      steps.push(`Número de pivotes (variables básicas): ${leadingOnes.size}`);
      steps.push(`Variables libres: ${freeVariables}`);
      steps.push('');
      steps.push(`Conclusión: El sistema tiene ${freeVariables} variable(s) libre(s).`);
      steps.push('Existen INFINITAS SOLUCIONES.');
      steps.push('La solución general depende de parámetros.');
      steps.push('');
      steps.push('════════════════════════════════════════════');
    ...
    // Solución única
    const solutionVector: number[] = new Array(m - 1).fill(0);
    steps.push(`Número de variables: ${m - 1}`);
    steps.push(`Número de pivotes: ${leadingOnes.size}`);
    steps.push(`Variables libres: 0`);
    steps.push('');
    steps.push('Conclusión: El sistema tiene SOLUCIÓN ÚNICA.');
    steps.push('');
    steps.push('SOLUCIÓN:');
    ...
    for (let i = 0; i < n; i++) {
      for (let j = 0; j < m - 1; j++) {
        if (Math.abs(matrix[i][j] - 1) < 0.0001) {
          solutionVector[j] = matrix[i][m - 1];
          steps.push(`  x${j + 1} = ${formatNumber(matrix[i][m - 1], useFractions)}`);
          break;
        }
      }
    }
    ...
    steps.push('');
    steps.push('Verificación: Sustituir estos valores en las ecuaciones');
    steps.push('originales para comprobar que satisfacen el sistema.');
    steps.push('');
    steps.push('════════════════════════════════════════════');
  ...
  try {
    const res = await fetch(`${API_BASE}/linear-systems/solve`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ augmented: matrixToSolve }),
    });
    ...
    if (data.error) {
      setSolution({
        type: (data.solution_type ?? 'none') as SolutionType,
        solution: null,
        steps: data.steps ?? [data.error],
      });
    } else {
      setSolution({
        type: (data.solution_type ?? 'unique') as SolutionType,
    ...
    setSolution({
      type: 'none',
      solution: null,
      steps: ['Error al conectarse al backend'],
    });
  }
};
                ...
                <div className="space-y-2">
                  {solution.solution.map((val, i) => (
                    <div key={i} className="flex items-center gap-3 text-green-200">
                      <span className="w-8 font-mono">x<sub>{i + 1}</sub></span>
                      <span>=</span>
                      <span className="px-3 py-1 bg-[#0d0d12]/50 rounded border border-green-500/30 font-mono">{formatNumber(val, useFractions)}</span>
                    </div>
                  ))}
                </div>
              </div>
