...
import { Calculator, Grid3x3, ListChecks, Box, Sigma, Trash2, Edit2, Sparkles, Binary, Hash, ChevronLeft, ChevronRight, Info } from 'lucide-react';
              ...
              {variables.map((variable) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-lg text-purple-100">
                        {variable.isVector ? 'Vector' : 'Matriz'} {variable.name}
                      </span>
                      {variable.isVector && variable.vector && (
                        <span className="text-xs px-2 py-0.5 bg-cyan-500/20 text-cyan-300 rounded-full border border-cyan-500/30">
                          {variable.vectorSize}D
                        </span>
                      )}
                      {!variable.isVector && variable.matrix && (
                  ...
                  <div className="mb-3 min-h-[70px] flex items-center justify-center bg-[#0d0d12]/50 rounded-lg border border-purple-500/20">
                    {variable.isVector && variable.vector ? (
                      // Vista previa de vector
                      <div className="text-xs p-2 max-w-full overflow-hidden">
                        {variable.vectorSize <= 5 ? (
                          // Mostrar vector completo si es pequeño
                          <div className="inline-flex flex-col gap-1">
                            {variable.vector.map((val, i) => (
                              <div key={i} className="flex items-center justify-center">
                                <span className="text-cyan-300 font-mono">
                                  {formatNumber(val, useFractions, 1)}
                                </span>
                              </div>
                            ))}
                          </div>
                        ) : (
                          // Indicador para vectores grandes
                          <div className="text-center text-cyan-400/50">
                            <div className="text-lg">⟨</div>
                            <div className="text-xs">Vector {variable.vectorSize}D</div>
                            <div className="text-lg">⟩</div>
                          </div>
                        )}
                      </div>
...
function formatNumber(value: number, useFractions: boolean, precision: number): string {
  if (useFractions) {
    const fraction = value.toString().split('.');
    if (fraction.length > 1) {
      const numerator = parseInt(fraction[0]) * Math.pow(10, fraction[1].length) + parseInt(fraction[1]);
