...
class AlgebraLinealGUI(ttk.Frame):
    ...
    def _build_matrices_tab(self):
        ...
        # Hecho: la pestaña ya existía; solo la estamos manteniendo y extendiendo
        ...
        # ---- NUEVAS OPCIONES: Homogéneo / No homogéneo y botón para hacer homogéneo ----
        self.hom_var = tk.BooleanVar(value=False)
        ttk.Label(cfg, text="Tipo:").grid(row=1, column=0, sticky="w", pady=(6,0))
        ttk.Radiobutton(cfg, text="No homogéneo", variable=self.hom_var, value=False).grid(row=1, column=1, sticky="w", pady=(6,0))
        ttk.Radiobutton(cfg, text="Homogéneo", variable=self.hom_var, value=True).grid(row=1, column=2, sticky="w", pady=(6,0))
        ttk.Button(cfg, text="Hacer homogéneo (b=0)", command=self._make_homogeneous).grid(row=1, column=3, padx=(8,0), pady=(6,0))
        # --------------------------------------------------------------------------------
    ...
    def _solve(self):
        ...
        # insertamos los pasos como antes
        self.txt.delete("1.0", tk.END)
        self.txt.insert(tk.END, salida)

        # --- Análisis paramétrico mejorado: rango, V.B., V.L., forma paramétrica, solución general ---
        try:
            rref = sistema.matriz  # rref matrix after elimination (list of rows)
            n_rows = len(rref)
            n_vars = len(rref[0]) - 1 if n_rows > 0 else 0
            eps = 1e-10

            # detectar pivotes por fila: primer columna no nula (incluyendo la columna aumentada)
            pivot_col_for_row = [-1] * n_rows
            for i in range(n_rows):
                for j in range(n_vars + 1):  # incluir columna aumentada para detectar filas inconsistentes
                    if abs(rref[i][j]) > eps:
                        pivot_col_for_row[i] = j
                        break

            # pivotes en A: columnas 0..n_vars-1 que aparecen como primer no nulo en alguna fila
            pivot_col_set_A = set()
            pivot_cols_rows = {}
            for i, c in enumerate(pivot_col_for_row):
                if c != -1 and c < n_vars:
                    pivot_col_set_A.add(c)
                    pivot_cols_rows[c] = i

            rank_A = len(pivot_col_set_A)
            # rango de aumentada = número de filas no nulas (es lo usual)
            rank_aug = sum(1 for i in range(n_rows) if pivot_col_for_row[i] != -1)

            # variables básicas y libres
            pivot_row_of_col = [-1] * n_vars
            for col, row in pivot_cols_rows.items():
                pivot_row_of_col[col] = row

            pivot_cols = [c for c in range(n_vars) if pivot_row_of_col[c] != -1]
            free_cols = [c for c in range(n_vars) if pivot_row_of_col[c] == -1]

            # particular solution (set free vars = 0)
            particular = [0.0] * n_vars
            for col in pivot_cols:
                prow = pivot_row_of_col[col]
                particular[col] = rref[prow][-1]

            # construir base del núcleo (para cada variable libre)
            basis = []
            for free in free_cols:
                vec = [0.0] * n_vars
                vec[free] = 1.0
                for pc in pivot_cols:
                    prow = pivot_row_of_col[pc]
                    if prow != -1:
                        coef = -rref[prow][free]
                        if abs(coef) > eps:
                            vec[pc] = coef
                basis.append((free, vec))

            # ahora presentar en el mismo formato visual que usas
            self.txt.insert(tk.END, "\n=== Análisis adicional (paramétrico) ===\n")
            self.txt.insert(tk.END, f"Rango(A) = {rank_A}\n")
            self.txt.insert(tk.END, f"Rango(A|b) = {rank_aug}\n")
            self.txt.insert(tk.END, f"n (nº incógnitas) = {n_vars}\n")
            self.txt.insert(tk.END, f"n - r = {n_vars - rank_A}\n\n")

            # mostrar V.B. y V.L.
            vb_names = [f"x{c+1}" for c in pivot_cols]
            vl_names = [f"x{c+1}" for c in free_cols]
            self.txt.insert(tk.END, "V.B.: " + (", ".join(vb_names) if vb_names else "—") + "\n")
            self.txt.insert(tk.END, "V.L.: " + (", ".join(vl_names) if vl_names else "—") + "\n\n")

            # incoherencia
            inconsistent = any(all(abs(rref[r][c]) < eps for c in range(n_vars)) and abs(rref[r][-1]) > eps for r in range(n_rows))
            if inconsistent:
                self.txt.insert(tk.END, "El sistema es inconsistente (sin solución).\n")
                self._status("Cálculo finalizado (inconsistente).")
                return

            # elegir nombres de parámetros según cuántas variables libres
            param_letters = []
            if len(free_cols) == 1:
                param_letters = ["s"]
            elif len(free_cols) == 2:
                param_letters = ["s", "t"]
            elif len(free_cols) == 3:
                param_letters = ["s", "t", "u"]
            else:
                # si hay más, usar s1, s2, ...
                param_letters = [f"s{i+1}" for i in range(len(free_cols))]

            # si es homogéneo (b == 0 en todas filas) o user selected hom_var
            is_hom_user = self.hom_var.get()
            # determine if augmented column is all zeros (actual homog.)
            augmented_all_zero = all(abs(rref[r][-1]) < eps for r in range(n_rows)) if n_rows>0 else True
            is_hom = is_hom_user or augmented_all_zero

            if is_hom:
                self.txt.insert(tk.END, "Sistema homogéneo — núcleo y forma paramétrica:\n\n")
                if len(basis) == 0:
                    self.txt.insert(tk.END, "El núcleo es {0} (única solución trivial x = 0).\n")
                else:
                    # mostrar V.B. y V.L. (ya mostrado) y relaciones (cada variable básica en función de libres)
                    for col in pivot_cols:
                        prow = pivot_row_of_col[col]
                        # construir expresión x_col = sum coef * free + 0 (no particular)
                        expr_parts = []
                        for idx_free, free_col in enumerate(free_cols):
                            coef = -rref[prow][free_col]
                            if abs(coef) > eps:
                                coef_fmt = (f"{int(coef)}" if float(coef).is_integer() else f"{coef:.4f}")
                                expr_parts.append(f"{coef_fmt}{param_letters[idx_free]}")
                        if expr_parts:
                            self.txt.insert(tk.END, f"x{col+1} = " + " + ".join(expr_parts) + "\n")
                        else:
                            self.txt.insert(tk.END, f"x{col+1} = 0\n")
                    for idx_free, free_col in enumerate(free_cols):
                        self.txt.insert(tk.END, f"x{free_col+1} = {param_letters[idx_free]}   (parámetro)\n")
                    self.txt.insert(tk.END, "\n")

                    # mostrar vectorial: x = s*(v) + t*(v2) ...
                    self.txt.insert(tk.END, "Forma vectorial (solución general):\n")
                    params_repr = []
                    for i, (free_idx, vec) in enumerate(basis):
                        pname = param_letters[i]
                        vec_str = "(" + ", ".join(f"{val:.4f}" for val in vec) + ")^T"
                        params_repr.append(f"{pname}{vec_str}")
                    self.txt.insert(tk.END, "x = " + " + ".join(params_repr) + "\n")
                    self.txt.insert(tk.END, "\nConjunto solución: espacio generado por dichos vectores.\n")
            else:
                # no homogéneo
                if len(basis) == 0:
                    # solución única
                    self.txt.insert(tk.END, "Solución única (particular):\n")
                    self.txt.insert(tk.END, "(" + ", ".join(f"{val:.4f}" for val in particular) + ")^T\n")
                else:
                    self.txt.insert(tk.END, "Soluciones infinitas — forma paramétrica:\n\n")
                    # mostrar cada variable básica como xp + sum coef*param
                    for col in pivot_cols:
                        prow = pivot_row_of_col[col]
                        parts = []
                        for idx_free, free_col in enumerate(free_cols):
                            ...
                            if abs(coef) > eps:
                                coef_fmt = (f"{int(coef)}" if float(coef).is_integer() else f"{coef:.4f}")
                                parts.append(f"{coef_fmt}{param_letters[idx_free]}")
                        const = particular[col]
                        const_fmt = (f"{int(const)}" if float(const).is_integer() else f"{const:.4f}")
                        if parts:
                            line = f"x{col+1} = {const_fmt} + " + " + ".join(parts)
                        else:
                            line = f"x{col+1} = {const_fmt}"
                        self.txt.insert(tk.END, line + "\n")
                    for idx_free, free_col in enumerate(free_cols):
                        self.txt.insert(tk.END, f"x{free_col+1} = {param_letters[idx_free]}   (parámetro)\n")
                    self.txt.insert(tk.END, "\n")

                    # mostrar vectorial x = particular + sum t_i * v_i
                    self.txt.insert(tk.END, "Una solución particular:\n")
                    self.txt.insert(tk.END, "(" + ", ".join(f"{val:.4f}" for val in particular) + ")^T\n\n")
                    self.txt.insert(tk.END, "Base del núcleo (vectores v_i):\n")
                    for free_idx, vec in basis:
                        self.txt.insert(tk.END, f"v_{free_idx+1} = ( " + ", ".join(f"{val:.4f}" for val in vec) + " )^T\n")
                    params_repr = []
                    for i, (free_idx, vec) in enumerate(basis):
                        pname = param_letters[i]
                        vec_str = "(" + ", ".join(f"{val:.4f}" for val in vec) + ")^T"
                        params_repr.append(f"{pname}{vec_str}")
                    self.txt.insert(tk.END, "\nSolución general:\n")
                    self.txt.insert(tk.END, "x = (" + ", ".join(f"{val:.4f}" for val in particular) + ")^T + " + " + ".join(params_repr) + "\n")

        except Exception as e:
            self._status(f"Error en análisis paramétrico: {e}")

        self._status("Cálculo finalizado.")

    def _make_homogeneous(self):
        # pone la columna b a cero en la grid actual (mantiene formato)
        try:
            m = int(self.var_m.get())
            n = int(self.var_n.get())
            for i in range(m):
                # última columna es index n
                e = self.entries[i][n]
                e.delete(0, tk.END)
                e.insert(0, "0")
            self._status("Columna b establecida en cero (homogéneo).")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo hacer homogéneo: {e}")
            self._status("Error al intentar hacer homogéneo.")
