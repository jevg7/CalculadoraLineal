...
class AlgebraLinealGUI(ttk.Frame):
    ...
    def _solve(self):
        ...
        # --- nuevo: análisis paramétrico y presentación en el mismo formato ---
        try:
            # matriz rref final en sistema.matriz (mutable)
            rref = sistema.matriz  # list of rows (floats)
            # variables
            n_rows = len(rref)
            n_vars = len(rref[0]) - 1 if n_rows > 0 else 0

            # detectar pivotes: para cada row buscar primer 1 (aprox) y comprobar que columna es pivot
            pivot_row_of_col = [-1] * n_vars
            for i in range(n_rows):
                # buscar primer índice no nulo en fila (excepto ultima columna)
                first = None
                ...
                if first is not None:
                    # verificar que en esa columna sea 1 y demás filas 0 (aprox)
                    if abs(rref[i][first] - 1) < 1e-8:
                        # check other rows zero in that col
                        if all(abs(rref[k][first]) < 1e-8 for k in range(n_rows) if k != i):
                            pivot_row_of_col[first] = i

            free_cols = [j for j in range(n_vars) if pivot_row_of_col[j] == -1]
            pivot_cols = [j for j in range(n_vars) if pivot_row_of_col[j] != -1]

            # particular solution (take free vars = 0): x_p has entries for pivot cols equal to last column at pivot rows
            particular = [0.0] * n_vars
            for col, prow in enumerate(pivot_row_of_col):
                if prow != -1:
                    particular[col] = rref[prow][-1]

            # generar base del núcleo (para cada variable libre, vector con 1 en esa libre y -coef en pivot positions)
            basis = []
            for free in free_cols:
                ...
                for pc in pivot_cols:
                    ...
                    if prow != -1:
                        # coeficiente en la fila prow, columna free
                        coef = -rref[prow][free]
                        # si es muy pequeño, ignora
                        if abs(coef) > 1e-12:
                            vec[pc] = coef
                basis.append((free, vec))

            # presentar resultados dependiendo de homogéneo/no homogéneo
            if self.hom_var.get():
                # force ensure augmented column is zero? user can choose 'Hacer homogéneo' to set b=0; but treat generally
                # For homogeneous system (b=0), particular should be zero vector.
                self.txt.insert(tk.END, "\n=== Análisis paramétrico (Sistema homogéneo) ===\n")
                if len(basis) == 0:
                    self.txt.insert(tk.END, "El núcleo es {0} (única solución trivial x = 0).\n")
                else:
                    self.txt.insert(tk.END, "Base del núcleo (vectores que generan las soluciones):\n")
                    for free_idx, vec in basis:
                        # mostrar vector en columna usando el mismo estilo
                        self.txt.insert(tk.END, f"v_{free_idx+1} = ( " + ", ".join(f"{val:.4f}" for val in vec) + " )^T\n")
                    params = [f"t{i+1}" for i in range(len(basis))]
                    combo = "x = " + " + ".join(f"{params[i]}·v_{basis[i][0]+1}" for i in range(len(basis))) + "\n"
                    self.txt.insert(tk.END, "\nSolución general:\n" + combo)
            else:
                # no homogéneo
                # check inconsistency: a row [0 ... 0 | c] with c != 0
                inconsistent = any(all(abs(rref[r][c]) < 1e-12 for c in range(n_vars)) and abs(rref[r][-1]) > 1e-12 for r in range(n_rows))
                if inconsistent:
                    self.txt.insert(tk.END, "\n=== Análisis paramétrico ===\nEl sistema es inconsistente (sin solución).\n")
                else:
                    if len(basis) == 0:
                        # solución única
                        self.txt.insert(tk.END, "\n=== Análisis paramétrico ===\nSolución única (particular):\n")
                        self.txt.insert(tk.END, "(" + ", ".join(f"{val:.4f}" for val in particular) + ")^T\n")
                    else:
                        self.txt.insert(tk.END, "\n=== Análisis paramétrico ===\nSoluciones infinitas.\n")
                        self.txt.insert(tk.END, "Una solución particular (variables libres = 0):\n")
                        self.txt.insert(tk.END, "(" + ", ".join(f"{val:.4f}" for val in particular) + ")^T\n\n")
                        self.txt.insert(tk.END, "Base del núcleo (vectores v_i):\n")
                        for free_idx, vec in basis:
                            self.txt.insert(tk.END, f"v_{free_idx+1} = ( " + ", ".join(f"{val:.4f}" for val in vec) + " )^T\n")
                        params = [f"t{i+1}" for i in range(len(basis))]
                        combo = "x = (" + ", ".join(f"{p:.4f}" for p in particular) + ")^T + " + " + ".join(f"{params[i]}·v_{basis[i][0]+1}" for i in range(len(basis))) + "\n"
                        self.txt.insert(tk.END, "\nSolución general:\n" + combo)

        except Exception as e:
            # si algo falla en análisis paramétrico, no bloqueamos la ejecución: simplemente mostrar error en status
